...
void init_shm(){
	if((speedid=shmget(SPEED_KEY, sizeof(int), 0666|IPC_CREAT))<0) exit(4);
	if((rpmid=shmget(RPM_KEY, sizeof(int), 0666|IPC_CREAT))<0) exit(4);
	if((fuelid=shmget(FUEL_KEY, sizeof(int), 0666|IPC_CREAT))<0) exit(4);
	if((coolantid=shmget(COOLANT_KEY, sizeof(int), 0666|IPC_CREAT))<0) exit(4);
	if((inputSpeedid=shmget(INPUT_SPEED_KEY, sizeof(int), 0666|IPC_CREAT))<0) exit(4);
	if((accid=shmget(ACC_KEY, sizeof(int), 0666|IPC_CREAT))<0) exit(4);
	
	if((speed = shmat(speedid,0,0))<0) exit(5);
	if((rpm = shmat(rpmid,0,0))<0) exit(5);
	if((fuel = shmat(fuelid,0,0))<0) exit(5);
	if((coolant = shmat(coolantid,0,0))<0) exit(5);
	if((inputSpeed = shmat(inputSpeedid,0,0))<0) exit(5);
	if((acc = shmat(accid,0,0))<0) exit(5);
...
void drawInfo(){
	...
	for(i=12; i>0; i--){
		...
		if(i%2!=0) printf("%3d", (NAME_SPACE-3-i)*20);
	}
	...
	for(i=12; i>0; i--){
		...
		if(i%2) printf("%3d", (NAME_SPACE-3-i)/2);
	}
		...
		if(sp<=160&&sp>=20){	
			...
			if(ac>=1&&ac<=10){
				...
				*fuelVal = 80;
				...
				if(pthread_create(&updateSpeedth, NULL, &updateSpeed, NULL)<0) exit(2);
				if(pthread_create(&updateRPMth, NULL, &updateRPM, NULL)<0) exit(2);
				if(pthread_create(&updateFuelth, NULL, &updateFuel, NULL)<0) exit(2);
				if(pthread_create(&updateCoolantth, NULL, &updateCoolant, NULL)<0) exit(2);
...
void *updateSpeed(void* arg){
	...
	int speedChart = 0;
	while(1){
		if(sync_th!=0) continue;
		pthread_mutex_lock(&cursorMutex);
		speedChart	= (int)((double)*speedVal/20);
		for(i=0;i<10;i++){
