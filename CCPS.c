...
void drawInfo() {
	...
	//속도 부분 초기화
	for (i = 10; i>0; i--) {
		move_cur(SPEED_X - 4, i + 5);//15 - 4, i + 5
		if (i % 2 != 0) printf("%3d", (NAME_SPACE - 3 - i) * 20);
	}
	move_cur(SPEED_X - 2, NAME_SPACE);//15 - 2, 16
	printf("%5s", "SPEED");
	move_cur(SPEED_X - 2, VALUE_SPACE);//15 - 2, 18
	printf("%3d", 0);

	//RPM부분 초기화
	for (i = 12; i>0; i--) {
		move_cur(RPM_X - 4, i + 3);// 30 - 4, i + 3
		if (i % 2) printf("%3d", (NAME_SPACE - 3 - i) / 2);//16 - 3 - i / 2
	}
	...
	//연료 부분 초기화
	move_cur(FUEL_X - 4, NAME_SPACE - 10);// 45 - 4, 16 - 10
	printf("%3c", 'F');
	move_cur(FUEL_X - 4, NAME_SPACE - 1);// 45 - 4, 16 - 1
	printf("%3c", 'E');
	move_cur(FUEL_X - 2, NAME_SPACE);// 45 - 2, 16
	printf("%5s", "FUEL");
	move_cur(FUEL_X - 2, VALUE_SPACE);// 45 - 2, 18
	printf("%3d", 0);

	//냉각수 부분 초기화
	move_cur(COOLANT_X - 2, NAME_SPACE);// 60 - 2, 16
	printf("%5s", "COOLANT");
	move_cur(COOLANT_X - 2, VALUE_SPACE);// 60 - 2, 18
	printf("%3d", 0);
}
...
void *updateSpeed(void* arg) {
	...
	while (1) {
		...
		for (i = 0; i<10; i++) {
			move_cur(SPEED_X, CHART_SPACE - i);//15, 15 - i
			printf("  ");
		}
...
void *updateFuel(void* arg) {
	...
	while (1) {
		...
		for (i = 0; i<10; i++) {
			move_cur(FUEL_X, CHART_SPACE - i);//45, 15 - i
			printf("  ");
		}
...
void *updateCoolant(void* arg) {
	...
	while (1) {
		...
		for (i = 0; i<10; i++) {
			move_cur(COOLANT_X, CHART_SPACE - i);//60, 15 - i
			printf("  ");
		}
...
void *Figures_RPM(void* arg) {
	...
	while (1) {
		...
		RPM = Speed * 1000000
			/ Min / Tire_Perimeter_Ratio; // 60 / 1850
...
void run_thread()
{
	...
	re = pthread_create(&thread1, NULL, Figures_Speed, NULL);//쓰레드 생성 기본
	if (re < 0) {
